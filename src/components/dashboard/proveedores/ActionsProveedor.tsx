...
import { Proveedor } from './proveedores-table';
...
export async function handleDeleteProveedor(id: number, reloadTable: () => void, notifyD: () => void) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.error(`Error al eliminar proveedor con ID ${id}:`, response);
        }
    } catch (error) {
...
export const EditModal: React.FC<EditModalProps> = ({ isOpen, onClose, proveedor, reloadTable, notifyA }: { isOpen: boolean, onClose: () => void, proveedor: Proveedor | null, reloadTable: () => void, notifyA: () => void }) => {
    ...
    useEffect(() => {
        if (proveedor) {
            setFormData({
                ...
                num_telefonico: proveedor.num_telefonico,
